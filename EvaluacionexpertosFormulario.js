$(document).ready(function() {
    ...
    //Boton de detalles
    crearBotonLink("cre0b_linkvistadetalle", "Detalles de nota conceptual");
    ...
    function agregarAyudasCriterios() {
        ...
        var ayudas = {
            ...
            "cre0b_verificacioncofinanciamiento": {
                contenido: [
    ...
    // ======= ESTILOS RECOMENDACION COLOR =======
    if (!document.getElementById("estilo-recomendacion")) {
    $("<style id='estilo-recomendacion'>\
        .estado-verde{\
    ...
    function calcularPuntajeEE() {
        ...
        var g1 = ((n("#cre0b_claridadobjetivos") + n("#cre0b_gradointegracion")) / 2) / 5 * 15;

        // Grupo 2 (20%) - 5 preguntas
        var g2 = (
            (n("#cre0b_alcanzabilidad") +
             n("#cre0b_aportevinculante") +
             n("#cre0b_aportenovinculante") +
             n("#cre0b_indicadoresrelevantes") +
             n("#cre0b_coherencia")) / 5
        ) / 5 * 20;

        // Grupo 3 (20%) - 4 preguntas
        var g3 = (
            (n("#cre0b_claridadproblematica") +
             n("#cre0b_coherenciasolucion") +
             n("#cre0b_adecuacionmetodologia") +
             n("#cre0b_nivelactividades")) / 4
        ) / 5 * 20;

        // Grupo 4 (10%) - 5 preguntas
        var g4 = (
            (n("#cre0b_trayectoriaorganizacion") +
             n("#cre0b_capacidadadministrativa") +
             n("#cre0b_formacionacademica") +
             n("#cre0b_experienciazona") +
             n("#cre0b_experienciaproyecto")) / 5
        ) / 5 * 10;

        // Grupo 5 (10%) - 1 pregunta
        var g5 = (n("#cre0b_consistenciah1") / 5) * 10;

        // Grupo 6 (5%) - 2 preguntas
        var g6 = ((n("#cre0b_garantizasostenibilidad") + n("#cre0b_verificacioncofinanciamiento")) / 2) / 5 * 5;

        // Grupo 7 (5%) - 1 pregunta
        var g7 = (n("#cre0b_sustentoreplicabilidad") / 5) * 5;

        // Grupo 8 (15%) - 1 pregunta
        var g8 = (n("#cre0b_presupuestoresultados") / 5) * 15;

        var total = g1 + g2 + g3 + g4 + g5 + g6 + g7 + g8;

        // Redondeo (puedes cambiar a 1 o 0 decimales si quieres)
        total = Math.round(total * 100) / 100;

        // Setear el campo Puntaje
       $("#cre0b_puntaje").val(total.toFixed(2).replace('.', ','));

        // opcional: que no lo editen a mano
        $("#cre0b_puntaje").prop("readonly", true);

        $("#cre0b_puntaje").trigger("change"); // para que se actualice la recomendación al cambiar el puntaje
    ...
    function toggleRecomendacionPorPuntaje() {

    var puntaje = n("#cre0b_puntaje");

    var $campo = $("#cre0b_recomendacionee");
    ...
    $campo.removeClass("estado-verde estado-rojo");
    
    // bloquear edición
    $campo.prop("readonly", true);

    if (puntaje >= 50) {
        $campo.val("Solicitar Propuesta Completa");
        $campo.addClass("estado-verde");
    } if (puntaje < 50) {
        $campo.val("No solicitar propuesta completa");
        $campo.addClass("estado-rojo");
    }
    }

    // Ejecutar cuando cambie el puntaje
    $("#cre0b_puntaje").on("input change", function () {
        toggleRecomendacionPorPuntaje();
    });

    //Actualiza el valor cada 1 segundos (1000 milisegundos)
    setInterval(function() {
        $("#cre0b_estadointernoee").val("277010002");//estado interno Completado
    ...
    function guardarDatos() {
        ...
        $("#cre0b_estadointernoee").val("277010001");
    ...
    function reglasBotonesEE() {
        var estadointernoee = $("#cre0b_estadointernoee").val();
        
        //estado intero es igual a enviado o a completado
        if (estadointernoee === "277010000" || estadointernoee === "277010002") {
            // se oculta
            ...
            $("#cre0b_estadointernoee").val("277010002");//setea a estado interno a enviado
        }
    ...
    $("#cre0b_estadointernoee").closest("td").hide();
